...
// Scoped tokeny majú užší účel než bežný JWT a obmedzujú, na ktorý krok ich možno použiť.
...
// Základná tabuľka používateľov drží OPAQUE registračný záznam
// aj pomocné biometrické a PQC údaje.
// 1) Základná tabuľka
db.prepare(`
...
if (!columnExists(db, "users", "pqc_sign_count_last")) {
  ...
  // SQLite použije DEFAULT len pri nových riadkoch; staršie záznamy môžu zostať NULL.
  // Preto ich normalizujeme na 0, aby nebolo treba dopĺňať COALESCE na každom mieste.
...
// Dočasný stav loginu si server drží v pamäti medzi štartom a dokončením OPAQUE protokolu.
...
app.post("/api/register/start", async (req, res) => {
  ...
  // userIdentifier je interný identifikátor, s ktorým pracuje OPAQUE server.
...
app.post("/api/register/finish", async (req, res) => {
  ...
  // Po úspešnej registrácii hesla vydáme token len pre ďalší krok, teda registráciu zariadenia.
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      // Backend iba sprostredkuje registráciu zariadenia; reálna práca so senzorom prebieha v lokálnom agente.
...
function pqcVerifyViaPython(args: {
  ...
  // 1) Nájdeme PROJECT_ROOT tak, že hľadáme tools/pqc_verify.py
  // Verifikáciu podpisu delegujeme do Python helpera, pretože tam máme dostupnú knižnicu pre ML-DSA.
  const rootsToTry = [
    process.cwd(),
  ...
  const PROJECT_ROOT = rootsToTry.find((r) =>
    fs.existsSync(path.resolve(r, "tools", "pqc_verify.py"))
  );

  if (!PROJECT_ROOT) {
    throw new Error(
      `project root not found; tried: ${rootsToTry.join(" | ")}`
    );
  }

  // Potom zostavíme absolútnu cestu k súboru pqc_verify.py.
  const scriptPath = path.resolve(PROJECT_ROOT, "tools", "pqc_verify.py");

  if (!fs.existsSync(scriptPath)) {
    throw new Error(`pqc_verify.py not found: ${scriptPath}`);
  }

  // Python interpreter hľadáme v premennej prostredia, lokálnom virtualenv alebo v systémovom Pythone.
...
function canonicalJson(obj: any): string {
  ...
  // Stabilné poradie kľúčov je dôležité, inak by server a agent hashovali odlišný obsah.
...
app.post("/api/pqc/verify", requireScope("pqc_2fa"), async (req, res) => {
  ...
  // Najprv overíme konzistenciu payloadu a výzvy, až potom ideme do kryptografickej verifikácie.
  await withSpan(
    {
      phase: "verify",
  ...
  // Agent podpisuje hash payloadu, preto ho server musí presne rovnako kanonizovať a zahashovať.
  ...
  // Po úspešnom overení označíme výzvu ako použitú a uložíme nové počítadlo podpisov.
  await withSpan(
    {
      phase: "db",
...
app.post("/api/login/start", async (req, res) => {
  ...
  // Prvý krok loginu pripraví serverovú odpoveď a zároveň si zapamätá dočasný stav protokolu.
...
app.post("/api/login/finish", async (req, res) => {
  ...
  // Po hesle ešte nevydávame finálny token, ale len dočasný token pre druhý faktor s PQC podpisom.
...
app.get("/api/admin/users", async (req, res) => {
  ...
  // Dashboard načítava len prehľadový výber údajov potrebných na administratívny pohľad.
...
// Celý backend publikujeme cez HTTPS, aby boli chránené všetky volania medzi klientom, serverom a agentom.
https.createServer(options, app).listen(PORT, "0.0.0.0", () => {
  console.log(`[OPAQUE] HTTPS (TLS1.3) server running on https://127.0.0.1:${PORT}`);
});

...
// Dashboard v raspberry variante získava údaje z backendu dostupného na tej
// istej doméne, z ktorej bol frontend načítaný.
const API = import.meta.env.VITE_API_BASE || `${window.location.origin}/api`;

export default function Dashboard() {
  // Meno prihláseného používateľa zobrazujeme z lokálneho úložiska.
  const username = localStorage.getItem("username");
  const navigate = useNavigate();

  // Zoznam používateľov aj príznak načítania sú lokálnym stavom obrazovky.
  const [users, setUsers] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);

  // Funkcia načíta údaje pre demonštračnú tabuľku používateľov.
  ...
  // Po prvom zobrazení stránky automaticky vyžiadame dáta z databázy.
  useEffect(() => {
    loadUsers();
  }, []);
  ...
  return (
    ...
    >
      ...
      <button
        onClick={() => {
          // Odhlásenie odstráni lokálne tokeny a používateľa vráti späť
          // na prihlasovaciu obrazovku.
          localStorage.clear();
          navigate("/");
        }}
        style={{
      ...
      {/* Počas načítania zobrazíme informačný text namiesto prázdnej plochy. */}
          ...
          <tbody>
            {users.map((u) => (
              <tr key={u.username} style={{ borderBottom: "1px solid rgba(255,255,255,0.1)" }}>
                ...
                {/* Dlhšie kryptografické reťazce skracujeme, aby bola tabuľka
                    lepšie čitateľná aj pri demonštrácii na menšej obrazovke. */}
